...
- Optional local STT via `whisper.cpp` is disabled by default. When `CORTANA_LOCAL_STT=true`, `/voice-turn` can use a configured local `whisper-cli.exe` and GGML model. If local STT is disabled or unavailable, the existing provider transcription path remains the default. Initial validation used whisper.cpp revision `4834a23` with `ggml-small.en.bin` SHA256 `C6138D6D58ECC8322097E0F987C32F1BE8BB0A18532A3F88F734D1BBF9C41E5D`. The executable and model are not committed to this repository.
...
- Optional local Silero VAD observation is disabled by default and does not replace provider `server_vad`, create/end turns, or control interruption. When explicitly enabled with `CORTANA_REALTIME_LOCAL_VAD=true`, it requires the optional `onnxruntime` dependency and a local ONNX model path supplied through `CORTANA_REALTIME_LOCAL_VAD_MODEL_PATH`. The ONNX model is not committed to this repository. The model used for the initial benchmark/live validation had SHA256 `1A153A22F4509E292A94E67D6F9B85E8DEB25B4988682B7E174C65279D8788E3`.
